...
add_action( 'wp_restore_post_revision', '\metarevisions\_wp_restore_post_revision_meta', 10, 2 );

// When creating or updating an autosave, save any revisioned meta fields.
add_action( 'wp_creating_autosave', '\metarevisions\_wp_autosave_post_revisioned_meta_fields' );
add_action( 'wp_updating_autosave', '\metarevisions\_wp_autosave_post_revisioned_meta_fields' );

// When creating a revision, also save any revisioned meta.
add_action( '_wp_put_post_revision', '\metarevisions\_wp_save_revisioned_meta_fields' );

//Filters
// When revisioned post meta has changed, trigger a revision save.
add_filter( 'wp_save_post_revision_post_has_changed', '\metarevisions\_wp_check_revisioned_meta_fields_have_changed', 10, 3 );


// Add the reviosioned meta to get_post_metadata for preview meta data.
function _add_metadata_preview_filter() {
	add_filter( 'get_post_metadata', '\metarevisions\_wp_preview_meta_filter', 10, 4 );
}
 ...
 */
function _wp_autosave_post_revisioned_meta_fields( $new_autosave ) {
	/**
 ...
 */
function wp_post_revision_meta_keys_for_revision( $revision_id = null ) {
	...
	if ( null !== $revision_id ) {
		...
		$revisioned_meta_keys = _wp_post_revision_meta_keys();
	}
 ...
 */
function _wp_check_revisioned_meta_fields_have_changed( $post_has_changed, \WP_Post $last_revision, \WP_Post $post ) {
	foreach ( _wp_post_revision_meta_keys() as $meta_key ) {
